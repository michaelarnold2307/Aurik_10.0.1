...
def analyze_only(audio: np.ndarray, sr: int, material: str = "unknown", era: int = 0) -> PreservationReport:
    ...
    if noise_ratio > 0.6: defects["hiss"] = noise_ratio

    # Clicks
    hf = np.sum(spec[freqs>=6000]**2) / max(np.sum(spec**2), 1e-10)
    if hf > 0.05: defects["clicks"] = float(hf)

    # Hum
    hum_e = sum(np.sum(spec[(freqs>=lo)&(freqs<=hi)]**2) for lo,hi in [(45,65),(95,125)])
    hum_r = hum_e / max(np.sum(spec**2), 1e-10)
    if hum_r > 0.1: defects["hum"] = float(hum_r)

    # Clipping
    clip_pct = float(np.mean(np.abs(mono)>0.95))
    if clip_pct > 0.01: defects["clipping"] = clip_pct

    # Strategie-Empfehlung
    strategies = []
    if defects: strategies.append("light")
    if len(defects) >= 2: strategies.append("balanced")
    if len(defects) >= 3: strategies.append("deep")
    ...
    return PreservationReport(
        material=material, era=era, defects_found=defects,
