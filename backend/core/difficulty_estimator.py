...
class DifficultyEstimate:
    ...
    def __post_init__(self):
        if self.defect_types is None: self.defect_types = []


def estimate(audio: np.ndarray, sr: int, material: str = "unknown", era: int = 0, defects: dict[str, float] = None) -> DifficultyEstimate:
    ...
    # Score-Berechnung
    score = 3  # Basis
    if material in ("shellac", "wax_cylinder"): score += 3
    elif material in ("vinyl", "reel_tape", "tape"): score += 2
    elif material in ("mp3_low", "cassette"): score += 1

    if noise_ratio > 0.7: score += 2
    elif noise_ratio > 0.5: score += 1

    if clipped_pct > 5: score += 2
    elif clipped_pct > 1: score += 1

    if rms_db < -30: score += 1  # Sehr leise
    ...
    return DifficultyEstimate(
        difficulty=difficulty, estimated_duration_minutes=round(est_duration, 1),
        ...
        material=material, era=era, recommendation=rec,
    )
