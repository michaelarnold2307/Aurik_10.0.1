...
SR = 22050; N_FFT = 1024; HOP = 256; N_MELS = 128; N_FRAMES = 128
...
class MUSDB18Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.from_numpy(audio_lim), torch.from_numpy(audio_seg.astype(np.float32)),
                torch.tensor(cutoff / 11025.0, dtype=torch.float32))

mel_t = torchaudio.transforms.MelSpectrogram(sample_rate=SR, n_fft=N_FFT, hop_length=HOP, n_mels=N_MELS, f_min=60, f_max=SR//2, center=False)
...
class Generator(nn.Module):
    def __init__(self, base_ch=24):
        super().__init__()
        C = base_ch
        self.enc1 = ConvBlock(1, C); self.enc2 = ConvBlock(C, C*2); self.enc3 = ConvBlock(C*2, C*4)
        self.pool = nn.MaxPool2d(2)
        self.bottleneck = ConvBlock(C*4, C*8)
        self.film_bn = FiLMBlock(1, C*8)
        self.up3 = nn.ConvTranspose2d(C*8, C*4, 2, 2); self.dec3 = ConvBlock(C*8, C*4)
        self.film_d3 = FiLMBlock(1, C*4)
        self.up2 = nn.ConvTranspose2d(C*4, C*2, 2, 2); self.dec2 = ConvBlock(C*4, C*2)
        self.up1 = nn.ConvTranspose2d(C*2, C, 2, 2); self.dec1 = ConvBlock(C*2, C)
    ...
    def forward(self, x, cutoff_norm):
        ...
        b = self.bottleneck(self.pool(e3)); b = self.film_bn(b, cutoff_norm)
        d3 = self.dec3(torch.cat([self.up3(b), e3], dim=1)); d3 = self.film_d3(d3, cutoff_norm)
...
class PatchGANDiscriminator(nn.Module):
    def __init__(self, in_channels=1, base_ch=32):
        ...
        self.layers = nn.Sequential(
            ...
            sn_conv(C*4, C*8, 4, 2, 1), nn.BatchNorm2d(C*8), nn.LeakyReLU(0.2, inplace=True),
            sn_conv(C*8, 1, 4, 1, 0),
        )
...
def multi_resolution_stft_loss(pred, target):
    ...
    for n_fft in [32, 64, 128]:
        for b in range(B):
            ps = torch.stft(pred[b,0,:,:], n_fft=n_fft, hop_length=n_fft//4, window=torch.hann_window(n_fft, device=pred.device), return_complex=True, pad_mode='reflect').abs()
            ts = torch.stft(target[b,0,:,:], n_fft=n_fft, hop_length=n_fft//4, window=torch.hann_window(n_fft, device=target.device), return_complex=True, pad_mode='reflect').abs()
            loss += F.l1_loss(ps, ts)
    return loss / (B*3)

def generator_loss(disc_fake): return -disc_fake.mean()
def discriminator_loss(disc_real, disc_fake): return F.relu(1.0 - disc_real).mean() + F.relu(1.0 + disc_fake).mean()
...
def train(data_dir="models/musdb18hq", epochs=100, batch_size=6, lr=1e-4, base_ch=24):
    ...
    print(f"Generator: {sum(p.numel() for p in generator.parameters()):,} params | Discriminator: {sum(p.numel() for p in discriminator.parameters()):,} params")
    ...
    if ckpt_path.exists():
        ...
        gs.update(pretrained); generator.load_state_dict(gs, strict=False)
    ...
    out_dir = Path("models/bw_reconstructor"); out_dir.mkdir(parents=True, exist_ok=True)
    lambda_stft = 0.3; lambda_gan_end = 0.1
    ...
    for epoch in range(epochs):
        ...
        total_g_loss = 0.0; total_d_loss = 0.0; t0 = time.time()
        for step, (x_audio, y_audio, cutoff_norm) in enumerate(dataloader):
            if step >= steps_per_epoch: break
            x_audio = x_audio.to(device); y_audio = y_audio.to(device)
            ...
            # Generator
            opt_g.zero_grad(); pred = generator(mel_x, cutoff_norm)
            ...
            g_loss.backward(); torch.nn.utils.clip_grad_norm_(generator.parameters(), 2.0); opt_g.step()
            # Discriminator
            d_loss = torch.tensor(0.0)
            if gan_weight > 0 and step % 2 == 0:
                opt_d.zero_grad()
                with torch.no_grad(): pred_detached = generator(mel_x, cutoff_norm).detach()
                d_loss = discriminator_loss(discriminator(mel_y), discriminator(pred_detached))
                d_loss.backward(); torch.nn.utils.clip_grad_norm_(discriminator.parameters(), 2.0); opt_d.step()
            total_g_loss += g_loss.item()
            if isinstance(d_loss, torch.Tensor): total_d_loss += d_loss.item()
        avg_g = total_g_loss / min(steps_per_epoch, step+1)
        avg_d = total_d_loss / max(1, (step+1)//2)
        sch_g.step(); elapsed = time.time() - t0
        print(f"Epoch {epoch+1:3d}/{epochs} | G:{avg_g:.4f} D:{avg_d:.4f} GAN:{gan_weight:.3f} | LR {sch_g.get_last_lr()[0]:.1e} | {elapsed:.0f}s", flush=True)
        if avg_g < best_loss:
            best_loss = avg_g
            torch.save({"model_state_dict": generator.state_dict(), "epoch": epoch, "loss": avg_g}, out_dir / "best_model_v4.pt")
    ...
    class Wrapper(nn.Module):
        def __init__(self, gen): super().__init__(); self.gen = gen
        def forward(self, x, cutoff): return self.gen(x, cutoff)
    export_model = Wrapper(generator)
    onnx_path = out_dir / "bw_reconstructor_v4.onnx"
    torch.onnx.export(export_model, (dummy_input, dummy_cutoff), str(onnx_path),
        input_names=["input", "cutoff"], output_names=["output"],
        dynamic_axes={"input": {0:"batch",2:"freq",3:"time"}, "cutoff": {0:"batch"}, "output": {0:"batch",2:"freq",3:"time"}}, opset_version=17)
    import onnx; onnx.checker.check_model(str(onnx_path))
