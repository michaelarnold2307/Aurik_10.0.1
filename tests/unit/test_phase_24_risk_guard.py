...
class TestPhase24RiskGuard:
    ...
    def test_01_dropout_evidence_includes_transport_bump(self):
        ...
        assert '"transport_bump"' in src, (
            "transport_bump fehlt in _dropout_evidence — "
            "Phase 24 könnte bei Transport-Bump-Material entfernt werden"
        )
    ...
    def test_02_dropout_evidence_includes_tape_head_level_dip(self):
        ...
        assert '"tape_head_level_dip"' in src, (
            "tape_head_level_dip fehlt in _dropout_evidence — "
            "Phase 24 könnte bei Head-Dip-Material entfernt werden"
        )
    ...
    def test_03_causal_probability_includes_both_defects(self):
        ...
        src = open(uv3_mod.__file__, encoding="utf-8").read()
        assert '"transport_bump"' in src, (
            "transport_bump fehlt in _causal_dropout_prob-Prüfung"
        )
        assert '"tape_head_level_dip"' in src, (
            "tape_head_level_dip fehlt in _causal_dropout_prob-Prüfung"
        )
    ...
    def test_04_phase_24_in_cause_to_phases_for_both_defects(self):
        ...
        assert "phase_24" in tb_section, (
            "transport_bump → phase_24 fehlt — keine Dropout-Reparatur für Bumps"
        )
        ...
        assert "phase_24" in th_section, (
            "tape_head_level_dip → phase_24 fehlt — keine Dropout-Reparatur für Dips"
        )
    ...
    def test_05_phase_24_allow_conditions_are_reasonable(self):
        ...
        assert "_dropout_evidence >= 0.20" in src, (
            "_dropout_evidence-Schwelle fehlt"
        )
        assert "_causal_dropout_prob >= 0.10" in src, (
            "_causal_dropout_prob-Schwelle fehlt"
        )
        assert "_pipe_conf >= 0.80" in src, (
            "_pipe_conf-Schwelle fehlt"
        )
        assert "restorability_score <= 35.0" in src, (
            "restorability_score-Schwelle fehlt"
        )
