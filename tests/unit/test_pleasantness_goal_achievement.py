...
class TestPleasantnessFirstPrinciple:
    ...
    def test_01_pleasantness_marker_registered(self):
        ...
        assert "pleasantness" in markers, (
            "pleasantness-Marker fehlt — §v10 nicht als Test-Kategorie erfasst"
        )
    ...
    def test_02_hpe_gate_threshold_and_skip(self):
        ...
        assert "-0.03" in src, (
            "HPE-Schwellwert -0.03 nicht in PMGG-Source — §v10 nicht implementiert"
        )
        assert "hpe_skip" in src, (
            "hpe_skip nicht in PMGG-Source — HPE-Gate kann Phasen nicht verwerfen"
        )
    ...
    def test_04_hpe_uses_psychoacoustic_dimensions(self):
        ...
        psychoacoustic_terms = [
            "roughness", "sharpness", "tonality", "naturalness",
            "pleasant", "zwicker", "ISO", "226", "loudness",
            "brightness", "warmth", "clarity", "masking", "bark",
            "ERB", "sone", "phon"
        ]
...
class TestGoalAchievementMatrix:
    ...
    def test_10_afg_uses_psychoacoustic_masking(self):
        ...
        psychoacoustic = [
            "masking_threshold", "roughness", "sharpness", "bark",
            "ERB", "ISO", "psychoacoustic", "Zwicker", "loudness"
        ]
        found = [t for t in psychoacoustic if t.lower() in src.lower()]
        assert len(found) >= 2, (
            f"AFG verwendet nur {len(found)} psychoakustische Terme ({found})"
        )
    ...
    def test_11_artifact_freedom_is_primary_veto(self):
        ...
        assert "artifact_freedom" in src, (
            "artifact_freedom fehlt in UV3 — §0h Veto-Faktor nicht implementiert"
        )
        has_rollback = "rollback" in src.lower() or "_rollback" in src.lower()
        assert has_rollback, (
            "Kein Rollback-Mechanismus — bei HPI ≤ 0 würde verschlechtertes Audio exportiert"
        )
