...
_REQUIRED_MATERIALS = {
    "wax_cylinder", "shellac", "vinyl", "reel_tape", "tape", "cassette",
    "cd_digital", "dat", "minidisc", "mp3_low", "mp3_high", "aac",
    "streaming", "wire_recording", "unknown",
}
...
class TestMaterialCeilingSync:
    ...
    def test_bw_ceiling_keys_match_ctc(self):
        ...
        assert bw_keys == ctc_keys, (
            f"BW keys mismatch: extra={bw_keys - ctc_keys}, missing={ctc_keys - bw_keys}"
        )

    def test_dr_ceiling_keys_match_ctc(self):
        ctc_keys = set(CARRIER_TRANSFER_CHARACTERISTICS.keys())
        dr_keys = set(MATERIAL_DR_CEILING_DB.keys())
        assert dr_keys == ctc_keys, (
            f"DR keys mismatch: extra={dr_keys - ctc_keys}, missing={ctc_keys - dr_keys}"
        )
    ...
    def test_bw_values_consistent_with_ctc(self):
        for mat, ctc_val in CARRIER_TRANSFER_CHARACTERISTICS.items():
            ...
            assert bw_from_dict == bw_from_ctc, (
                f"{mat}: BW dict={bw_from_dict} but CTC[0]={bw_from_ctc}"
            )
    ...
    def test_dr_values_consistent_with_ctc(self):
        for mat, ctc_val in CARRIER_TRANSFER_CHARACTERISTICS.items():
            ...
            assert dr_from_dict == dr_from_ctc, (
                f"{mat}: DR dict={dr_from_dict} but CTC[3]={dr_from_ctc}"
            )
