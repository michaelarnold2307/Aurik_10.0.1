...
def test_post_pipeline_collapse_guard_thresholds_present() -> None:
    ...
    assert (
        "if _cu2_imb > 20.0 and _pp2_imb < 6.0" in text
...
def test_collapse_guard_recovery_cascade_present() -> None:
    ...
    assert (
        "_afg_best_clean_checkpoint" in text
    ...
    assert (
        "best_clean_checkpoint selbst kollabiert" in text
...
def test_hpi_rollback_checkpoint_stereo_health_validation_present() -> None:
    ...
    assert (
        "HPI-Rollback-Checkpoint hat silent R-Kanal" in text or "HPI-Rollback-Checkpoint" in text
