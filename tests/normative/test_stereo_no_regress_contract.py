...
def test_post_pipeline_collapse_guard_thresholds_present() -> None:
    ...
    assert "§2.49b POST-PIPELINE cumulative stereo collapse" in text, (
        "§2.49b Guard-Block fehlt in UV3."
    )
...
def test_collapse_guard_recovery_cascade_present() -> None:
    ...
    assert "pre_pipeline_audio" in text, (
        "Stereo-Collapse-Guard braucht Fallback auf pre_pipeline_audio."
    )
...
def test_hpi_rollback_checkpoint_stereo_health_validation_present() -> None:
    ...
    assert "_hpi_best_rollback_audio" in text, (
        "HPI-Rollback-Ziel muss als dediziertes Checkpoint-Feld vorhanden sein."
    )
