...
def measure_lag(audio, sr, label):
    ...
    from backend.file_import import _estimate_interchannel_lag_samples, _estimate_interchannel_lag_multi_point
    single = _estimate_interchannel_lag_samples(audio, sr)
    multi = _estimate_interchannel_lag_multi_point(audio, sr, num_points=5)
    print(f"  [{label}] single={single:>6} samples ({single/sr*1000:>6.1f}ms) | "
...
def test_import_stage():
    ...
    audio = result["audio"]
    sr = result["sr"]
    print(f"  Loaded: shape={audio.shape}, sr={sr}, lag_before={result.get('interchannel_lag_samples_before')}, lag_after={result.get('interchannel_lag_samples_after')}")
