...
@dataclass
class Result:
    tool: str; tool_name: str; scenario: str
    pqs_aurik: float; pqs_tool: float; pqs_delta: float
    timbre: float; artifact_free: float
    runtime_a: float; runtime_t: float
    error: str|None = None
...
TOOLS = {"librosa_hpss": tool_librosa_hpss, "scipy_wiener": tool_scipy_wiener,
         "scipy_lowpass": tool_scipy_lowpass, "numpy_median": tool_numpy_median}
...
def run_scenario(name, clean, degraded, tool_key, sr=48000):
    ...
    try:
        ...
        return Result(tool=tool_key, tool_name=info["name"], scenario=name,
                      pqs_aurik=0, pqs_tool=0, pqs_delta=0,
                      timbre=0, artifact_free=0, runtime_a=ta, runtime_t=0, error=str(e))
    
    pqs_a = compute_perceptual_quality(clean, aurik_out)
    pqs_t = compute_perceptual_quality(clean, tool_out)
    
    return Result(tool=tool_key, tool_name=info["name"], scenario=name,
                  pqs_aurik=round(pqs_a,2), pqs_tool=round(pqs_t,2),
                  ...
                  runtime_a=round(ta,4), runtime_t=round(tt,4))


def run(tools=None, dur=1.0):
    import datetime
    if tools is None: tools = list(AVAILABLE_TOOLS)
    ...
    s = {
        "total": len(results), "ok": len(ok), "failed": len(results)-len(ok),
...
def main():
    ...
    print(f"   Metrik: PQS (Perceptual Quality Score 0-100)")
    ...
    for r in results:
        if r.error:
            ...
            print(f"  [{r.scenario:<15}] {r.tool_name:<30} "
    ...
    if a.output:
        ...
        out.write_text(json.dumps({"summary": s, "results": [
            {"tool": r.tool, "scenario": r.scenario, "pqs_aurik": r.pqs_aurik,
             "pqs_tool": r.pqs_tool, "pqs_delta": r.pqs_delta,
             "timbre": r.timbre, "artifact_free": r.artifact_free,
             "runtime_a": r.runtime_a, "runtime_t": r.runtime_t, "error": r.error}
